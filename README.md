...
describe LogStash::PipelineAction:Reload do
  ...
  after do
    pipeline.running_pipelines do |_, pipeline|
      pipeline.shutdown
  ...
  context "when existing pipeline and new pipeline are both reloadable" do
    it "stop the previous pipeline" do
      expect { subject.execute(agent, pipelines) }.to change(pipeline, :running?).from().to(false)
    end
  ...
  context "when the new pipeline has syntax errors" do
    let(:new_pipeline_config) { mock_pipeline_config(pipeline_id, "input dummyblockinput { id => 'new' } } output { null {} }", {"pipeline.reloadable" => false}) }
  ...
  context "when there is an error in the register" do
    ...
    it "cannot sucessfully execute the action" do
      expect(subject.execute(agent, pipelines)).not_to be_a_successful_action
    end
  end
end
